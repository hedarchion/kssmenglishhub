        ...
        <CardContent className="p-2 pt-0">
          <div className="space-y-1">
            {pupilsProfile.map((profile, index) => (
              <div key={index} className="group flex items-start gap-2 p-1.5 rounded hover:bg-muted/50">
                <Badge className="text-xs px-2 py-0.5 h-5 flex-shrink-0">{profile.name}</Badge>
        ...
        <CardContent className="p-2 pt-0">
          <div className="space-y-1">
            {hotsLevels.map((level, index) => (
              <div key={index} className="group flex items-start gap-2 p-1.5 rounded hover:bg-muted/50">
                <Badge variant="secondary" className="text-xs px-2 py-0.5 h-5 flex-shrink-0">{level.level}</Badge>
          ...
          <div className="flex flex-wrap gap-1.5">
            {crossCurricularElements.map((element, index) => (
              <div key={index} className="group">
                <Badge variant="outline" className="text-sm px-2.5 py-1 flex items-center gap-1 hover:bg-primary/10">
                  {element}
        ...
        <CardContent className="p-2 pt-0">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
            {[
              ...
              <div key={index} className="flex items-center justify-between p-2.5 rounded border hover:border-primary/30 group">
